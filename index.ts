...
`,
			...
			build.onResolve(
				{ filter: /^__inline-worker$/ },
				({ path, pluginData }) => {
					return { path, namespace: "inline-worker", pluginData };
				}
			);
			build.onLoad({ filter: /.*/, namespace: "inline-worker" }, ({pluginData: {workerPath}}) => {
				const options: WorkerOptions = {
					name:
						typeof workerPluginConfig.workerName === "function"
							? workerPluginConfig.workerName({ workerPath })
							: workerPluginConfig.workerName || undefined,
